...
SOURCE_COMMIT = '7111f4e35847adc4a172354ad5fabe485c1cfc47'
RELEASE_NOTES = (
    'Dienstplan auf Mobilgeräten verbessert: letzte Schichten bleiben vollständig '
    'oberhalb der festen Navigation sichtbar; zusätzliche Stabilitätskorrekturen '
    'für Kunden-, Standort- und Positionszuordnungen.'
)


def queue_scroll_fix_release(apps, schema_editor):
    ...
    app.review_notes = (
        ...
        'This update improves the mobile schedule so the last shift cards remain fully visible '
        'above the fixed bottom navigation and includes customer/location assignment stability fixes. '
    ...
    if localization:
        localization.release_notes = (
            'Dienstplan verbessert: letzte Schichten bleiben vollständig sichtbar; '
            'Kunden-, Standort- und Positionszuordnungen stabilisiert.'
        )
    ...
    build_number = max(int(db_max) + 1, 21)
    ...
    for platform, job_type, runner in (
        ...
        Job.objects.create(
            ...
            build=build,
            payload={
                'source': 'dienstplan-bottom-clearance-20260906',
...
class Migration(migrations.Migration):
    dependencies = [
        ('publisher', '0017_queue_aplus_solution_customer_color_release'),
    ]

    operations = [
        migrations.RunPython(queue_scroll_fix_release, noop_reverse),
    ]
